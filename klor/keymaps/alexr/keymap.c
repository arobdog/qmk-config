/*

                                        █         █    █            ▄▄▄▀▀▀▀▀▀▄▄▄        █▀▀▀▀▀▀▀▀▀▀█
                                        █        █     █          ▄▀            ▀▄      █          █
                                        █       █      █        ▄▀                ▀▄    █          █
                                        █      █       █        █                  █    █          █
                                        █     █        █       █                    █   █          █
                                        █    █         █       █                    █   █▄▄▄▄▄▄▄▄▄▄█
                                        █   █ █        █       █                    █   █      █
                                        █  █   █       █        █                  █    █       █
                                        █ █     █      █        ▀▄                ▄▀    █        █
                                        ██       █     █          ▀▄            ▄▀      █         █
                                        █         █    █▄▄▄▄▄▄▄▄    ▀▀▀▄▄▄▄▄▄▀▀▀        █          █

                                        ▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
                                        D E F A U L T

*/
#include QMK_KEYBOARD_H
#include <stdio.h>
#include <string.h>
#include "klor.h"
#ifdef HAPTIC_ENABLE
#include "drivers/haptic/DRV2605L.h"
#endif // HAPTIC ENABLE

// Define layer indices
enum klor_layers {
    _COLEMAK_DHM = 0,
    _UPPER,
    _LOWER,
    _ADJUST,
};

// Define custom keycodes
enum custom_keycodes {
    COLEMAK_DHM = SAFE_RANGE,
    UPPER,
    LOWER,
    ADJUST,
    OS_SWAP,
    MAKE_H,
};

// Define home row mods
#define GUI_A MT(MOD_LGUI, KC_A)
#define ALT_R MT(MOD_LALT, KC_R)
#define CTL_S MT(MOD_LCTL, KC_S)
#define SHT_T MT(MOD_LSFT, KC_T)
#define SHT_N MT(MOD_RSFT, KC_N)
#define CTL_E MT(MOD_RCTL, KC_E)
#define ALT_I MT(MOD_RALT, KC_I)
#define GUI_O MT(MOD_RGUI, KC_O)

// Define sounds
#ifdef AUDIO_ENABLE
#define WINXP_SOUND W__NOTE(_DS6), Q__NOTE(_DS5), H__NOTE(_AS5), H__NOTE(_GS5), H__NOTE(_DS5), H__NOTE(_DS6), H__NOTE(_AS5)
#define MAC_SOUND S__NOTE(_CS5), B__NOTE(_C5)
float winxp_song[][2] = SONG(WINXP_SOUND);
float mac_song[][2] = SONG(MAC_SOUND);
#endif // AUDIO_ENABLE

// Define keymaps
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_COLEMAK_DHM] = LAYOUT_polydactyl(
        KC_Q, KC_W, KC_F, KC_P, KC_B, KC_J, KC_L, KC_U, KC_Y, KC_SCLN,
        KC_TAB, GUI_A, ALT_R, CTL_S, SHT_T, KC_G, KC_H, SHT_N, CTL_E, ALT_I, GUI_O, KC_QUOT,
        KC_LCTL, KC_Z, KC_X, KC_C, KC_D, KC_V, KC_MUTE, KC_MPLY, KC_K, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RALT,
        KC_LGUI, MO(_LOWER), KC_SPC, KC_LSFT, KC_ESC, KC_ENT, MO(_UPPER), KC_BSPC
    ),
    [_UPPER] = LAYOUT_polydactyl(
        KC_CAPS, KC_7, KC_8, KC_9, KC_LBRC, KC_RBRC, RALT(KC_3), KC_ASTR, KC_CIRC, XXXXXXX,
        KC_HOME, KC_0, KC_4, KC_5, KC_6, KC_LCBR, KC_RCBR, KC_MINS, KC_EQL, KC_AMPR, KC_GRV, TO(_ADJUST),
        KC_END, XXXXXXX, KC_1, KC_2, KC_3, KC_LPRN, KC_MUTE, KC_MPLY, KC_RPRN, KC_UNDS, KC_PLUS, KC_PIPE, KC_TILD, XXXXXXX,
        KC_LGUI, MO(_LOWER), XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, MO(_UPPER), KC_BSPC
    ),
    [_LOWER] = LAYOUT_polydactyl(
        XXXXXXX, KC_AT, KC_HASH, KC_DLR, KC_PERC, KC_CIRC, KC_AMPR, XXXXXXX, XXXXXXX, KC_BSLS,
        XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, TO(_ADJUST),
        XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_MUTE, KC_MPLY, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
        XXXXXXX, MO(_LOWER), XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, MO(_UPPER), XXXXXXX
    ),
    [_ADJUST] = LAYOUT_polydactyl(
        AU_TOGG, XXXXXXX, RGB_HUI, RGB_MOD, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
        QK_BOOT, DB_TOGG, XXXXXXX, RGB_SAI, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
        MAKE_H, OS_SWAP, COLEMAK_DHM, RGB_VAI, XXXXXXX, XXXXXXX, KC_MUTE, KC_MPLY, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
        TO(_COLEMAK_DHM), TO(_COLEMAK_DHM), TO(_COLEMAK_DHM), TO(_COLEMAK_DHM), TO(_COLEMAK_DHM), TO(_COLEMAK_DHM), TO(_COLEMAK_DHM), TO(_COLEMAK_DHM)
    )
};

// Haptic Feedback
void keyboard_post_init_user(void) {
#ifdef HAPTIC_ENABLE
    haptic_disable(); // Disables per key haptic feedback by default
#endif // HAPTIC ENABLE
}

// OLED Configuration
#ifdef OLED_ENABLE

// Define variables for dynamic macros and OLED
// static uint16_t dmacro_timer = 0;
// static char *dmacro_text[] = {"Recording", "Stopped", "Playing", NULL};
char layer_state_str[24] = "BASE COLEMAK_DHM";
char o_text[24] = "";
int dmacro_num = 0;

// Define OLED icons
static const char PROGMEM rec_ico[] = {0x00, 0x00, 0x7c, 0x44, 0x44, 0x7c, 0x00, 0x00};
static const char PROGMEM stop_ico[] = {0x00, 0x00, 0x7e, 0x7e, 0x7e, 0x7e, 0x00, 0x00};
static const char PROGMEM play_ico[] = {0x00, 0x00, 0x40, 0x60, 0x70, 0x60, 0x40, 0x00};

void matrix_scan_user(void) {
#ifdef DYNAMIC_MACRO_ENABLE
    if (dmacro_num > 0) {
        if (timer_elapsed(dmacro_timer) < 3000) {
            strcpy(o_text, dmacro_text[dmacro_num - 1]);
        } else {
            strcpy(o_text, layer_state_str);
            dmacro_num = 0;
        }
    }
#endif // DYNAMIC_MACRO_ENABLE
}

void render_os_lock_status(void) {
    static const char PROGMEM sep_v[] = {0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0xC0, 0};
    static const char PROGMEM sep_h1[] = {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0};
    static const char PROGMEM sep_h2[] = {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0};
    static const char PROGMEM face_1[] = {0x80, 0x81, 0x82, 0x83, 0x84, 0xE1, 0};
    static const char PROGMEM face_2[] = {0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xE1, 0};
    static const char PROGMEM os_m_1[] = {0x95, 0x96, 0};
    static const char PROGMEM os_m_2[] = {0xB5, 0xB6, 0};
    static const char PROGMEM os_w_1[] = {0x97, 0x98, 0};
    static const char PROGMEM os_w_2[] = {0xB7, 0xB8, 0};
    static const char PROGMEM s_lock[] = {0x8F, 0x90, 0};
    static const char PROGMEM n_lock[] = {0x91, 0x92, 0};
    static const char PROGMEM c_lock[] = {0x93, 0x94, 0};
    static const char PROGMEM b_lock[] = {0xE1, 0xE1, 0};
#ifdef AUDIO_ENABLE
    static const char PROGMEM aud_en[] = {0xAF, 0xB0, 0};
    static const char PROGMEM aud_di[] = {0xCF, 0xD0, 0};
#endif
#ifdef HAPTIC_ENABLE
    static const char PROGMEM hap_en[] = {0xB1, 0xB2, 0};
#endif

    oled_write_ln_P(sep_v, false);
    if (keymap_config.swap_lctl_lgui) {
        oled_write_P(os_m_1, false);
    } else {
        oled_write_P(os_w_1, false);
    }
    oled_write_P(sep_h1, false);
    oled_write_P(face_1, false);
    if (keymap_config.swap_lctl_lgui) {
        oled_write_P(os_m_2, false);
    } else {
        oled_write_P(os_w_2, false);
    }
    oled_write_P(sep_h1, false);
    oled_write_P(face_2, false);
    oled_write_ln_P(sep_v, false);

    led_t led_usb_state = host_keyboard_led_state();
    if (led_usb_state.num_lock) {
        oled_write_P(n_lock, false);
    } else {
        oled_write_P(b_lock, false);
    }
    if (led_usb_state.caps_lock) {
        oled_write_P(c_lock, false);
    } else {
        oled_write_P(b_lock, false);
    }
    if (led_usb_state.scroll_lock) {
        oled_write_P(s_lock, false);
    } else {
        oled_write_P(b_lock, false);
    }

    oled_write_P(sep_h2, false);
#ifndef AUDIO_ENABLE
    oled_write_P(b_lock, false);
#endif
#ifndef HAPTIC_ENABLE
    oled_write_P(b_lock, false);
#endif
#ifdef AUDIO_ENABLE
    if (is_audio_on()) {
        oled_write_P(aud_en, false);
    } else {
        oled_write_P(aud_di, false);
    }
#endif
#ifdef HAPTIC_ENABLE
    oled_write_P(hap_en, false);
#endif
}

layer_state_t layer_state_set_kb(layer_state_t state) {
    switch (get_highest_layer(state | default_layer_state)) {
        case _COLEMAK_DHM:
            strcpy(layer_state_str, "BASE COLEMAK_DHM");
            break;
        case _UPPER:
            strcpy(layer_state_str, "UPPER");
            break;
        case _LOWER:
            strcpy(layer_state_str, "LOWER");
            break;
        case _ADJUST:
            strcpy(layer_state_str, "ADJUST");
            break;
        default:
            strcpy(layer_state_str, "XXXXXX");
    }
    if (dmacro_num < 1) {
        strcpy(o_text, layer_state_str);
    }
    return update_tri_layer_state(state, _UPPER, _LOWER, _ADJUST);
}

bool oled_task_kb(void) {
    if (!oled_task_user()) {
        return false;
    }
    if (is_keyboard_master()) {
#ifdef DYNAMIC_MACRO_ENABLE
        if (dmacro_num == 1) oled_write_P(rec_ico, false);
        if (dmacro_num == 2) oled_write_P(stop_ico, false);
        if (dmacro_num == 3) oled_write_P(play_ico, false);
#endif
        oled_write_ln(o_text, false);
        render_os_lock_status();
    } else {
        // 128x32 pixel bitmap of a stylized rocket ship (512 bytes)
        static const char PROGMEM rocket_ship[] = {
            // Row 0 (Top of the screen - empty space)
            ,
            // Row 1 (Rocket nose tip)
            ,
            // Row 2 (Rocket nose widening)
            ,
            // Row 3 (Rocket body starts)
            
        };
        oled_write_raw_P(rocket_ship, sizeof(rocket_ship));
    }
    return false;
}
#endif // OLED_ENABLE

uint16_t get_tapping_term(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case SHT_T:
            return TAPPING_TERM - 150;
        case SHT_N:
            return TAPPING_TERM - 150;
        default:
            return TAPPING_TERM;
    }
}

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case OS_SWAP:
            if (record->event.pressed) {
                if (!keymap_config.swap_lctl_lgui) {
                    keymap_config.swap_lctl_lgui = true;
#ifdef AUDIO_ENABLE
                    PLAY_SONG(mac_song);
#endif
                } else {
                    keymap_config.swap_lctl_lgui = false;
#ifdef AUDIO_ENABLE
                    PLAY_SONG(winxp_song);
#endif
                }
#ifdef HAPTIC_ENABLE
                DRV_pulse(pulsing_strong);
#endif
                eeconfig_update_keymap(keymap_config.raw);
                clear_keyboard();
                return false;
            }
            break;

        case COLEMAK_DHM:
            if (record->event.pressed) {
                set_single_persistent_default_layer(_COLEMAK_DHM);
#ifdef HAPTIC_ENABLE
                DRV_pulse(transition_hum);
#endif
            }
            return false;

        case UPPER:
            if (record->event.pressed) {
                layer_on(_UPPER);
                update_tri_layer(_UPPER, _LOWER, _ADJUST);
            } else {
                layer_off(_UPPER);
                update_tri_layer(_UPPER, _LOWER, _ADJUST);
            }
            return false;

        case LOWER:
            if (record->event.pressed) {
                layer_on(_LOWER);
                update_tri_layer(_UPPER, _LOWER, _ADJUST);
            } else {
                layer_off(_LOWER);
                update_tri_layer(_UPPER, _LOWER, _ADJUST);
            }
            return false;

        case ADJUST:
            if (record->event.pressed) {
                layer_on(_ADJUST);
            } else {
                layer_off(_ADJUST);
            }
            return false;

        case MAKE_H:
            if (record->event.pressed) {
#ifdef KEYBOARD_klor_kb2040
                SEND_STRING("qmk compile -kb klor/2040 -km alexr");
#else
                SEND_STRING("qmk compile -kb klor -km alexr");
#endif
                tap_code(KC_ENTER);
            }
            break;

        case KC_MPLY:
            if (record->event.pressed) {
#ifdef HAPTIC_ENABLE
                DRV_pulse(sharp_click);
#endif
            }
            break;
    }
    return true;
}

#ifdef ENCODER_ENABLE
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) {
        if (clockwise) {
            tap_code(KC_WH_U);
        } else {
            tap_code(KC_WH_D);
        }
    } else if (index == 1) {
        if (IS_LAYER_ON(_LOWER)) {
            if (clockwise) {
                tap_code(KC_MNXT);
            } else {
                tap_code(KC_MPRV);
            }
        } else {
            if (clockwise) {
                tap_code(KC_VOLU);
            } else {
                tap_code(KC_VOLD);
            }
        }
    }
    return true;
}
#endif // ENCODER_ENABLE

/*

                                                       ▐█    ▟▛ ▐█     ▄▆▀▀▀▀▀▀▆▄  ▐█▀▀▀▀▀█▌
                                                       ▐█   ▟▛  ▐█    ▟▛        ▜▙ ▐█     █▌
                                                       ▐█  ▟▛   ▐█   ▐█          █▋▐█     █▌
                                                       ▐█ ▟█▙   ▐█   ▐█          █▋▐█▀▀▜█▀▀▘
                                                       ▐█▟▛ ▜▙  ▐█    ▜▙        ▟▛ ▐█   ▜▙
                                                       ▐█▛   ▜▙ ▐█▄▄▄▄ ▀▜▆▄▄▄▄▆▛▀  ▐█    ▜▙

                                                                 ▄██████████████▄
                                                                 ████████████████
                                                            ▄██████▀  ▀████▀  ▀██████▄
                                                            ███████▄  ▄████▄  ▄███████
                                                            ███████████▀▀▀▀███████████
                                                            ▀█████████▀ ▄▄ ▀█████████▀
                                                                 ████▀ ▄██▄ ▀████
                                                                 ████▄▄████▄▄████

*/
